...
// Malawak na listahan ng language codes (kapareho ng suportadong wika sa Language panel).
// Ginagamit ito ng fetchTrailer() para hilingin sa TMDB ang mga video na naka-tag sa
// KAHIT ANONG wika sa mundo, dahil madalas naka-tag lang ang trailers sa original
// language ng title (hal. Spanish para sa Marimar, Japanese para sa anime, atbp.)
// sa halip na English.
const ALL_LANG_CODES = [
    "en","tl","ja","ko","zh","es","fr","id","ar","hi","pt","de","it","ru",
    "vi","th","tr","nl","pl","sv","no","da","fi","el","he","uk","cs","hu",
    "ro","bn","ur","ms","sw","fa","pa","ta","my","km"
];
...
let currentSelectedShow = { id: "", type: "", youtubeKey: "", title: "" };
...
function fetchFullDetails(id, type, autoplay = false) {
    const detailsUrl = `${BASE_URL}/${type}/${id}?append_to_response=credits&language=${tmdbLang()}`;

    // Sabay na kukunin ang details (cast/desc) at ang trailer (hiwalay na request
    // dahil mas malawak ang language coverage kapag hiwalay itong hinihingi).
    // Gumagamit ng simpleng counter para sabay pareho bago mag-autoplay.
    let pending = 2;
    const tryFinish = () => {
        pending--;
        if (pending === 0 && autoplay) openPlayer();
    };
    ...
    fetchTrailer(id, type, tryFinish);
...
// Hiwalay na pagkuha ng trailer/teaser sa YouTube.
// TMDB by default: kapag "language=X" lang, videos na naka-tag lang sa "X" ang lalabas.
// Maraming titulo (anime, telenovela tulad ng Marimar, atbp.) na walang laman sa "en"
// pero meron sa original language nila. Kaya dito, hinihiling natin ang videos sa
// HALOS LAHAT ng language (ALL_LANG_CODES) sa isang request, bago tuluyang mag-fallback
// sa "walang trailer" state.
function fetchTrailer(id, type, callback) {
    currentSelectedShow.youtubeKey = "";

    // priority order: Trailer > Teaser > Clip/Featurette (kaysa wala talaga)
    const pickBest = (results) => {
        if (!results) return null;
        return results.find(v => v.site === "YouTube" && v.type === "Trailer")
            || results.find(v => v.site === "YouTube" && v.type === "Teaser")
            || results.find(v => v.site === "YouTube" && (v.type === "Clip" || v.type === "Featurette"))
            || results.find(v => v.site === "YouTube")
            || null;
    };

    // Unang subok: HALOS LAHAT ng language + walang naka-tag na wika (null)
    const includeLangs = [...new Set([currentLanguage, ...ALL_LANG_CODES])].join(",");
    const primaryUrl = `${BASE_URL}/${type}/${id}/videos?language=${tmdbLang()}&include_video_language=${includeLangs},null`;

    fetchData(primaryUrl, (data) => {
        const best = pickBest(data && data.results);
        if (best) {
            currentSelectedShow.youtubeKey = best.key;
            if (callback) callback();
            return;
        }

        // Pangalawang subok: default en-US na TMDB videos list (paiba-ibang response minsan
        // depende sa endpoint variant, kaya subukan din ito bago sumuko)
        fetchData(`${BASE_URL}/${type}/${id}/videos?language=en-US`, (data2) => {
            const best2 = pickBest(data2 && data2.results);
            if (best2) currentSelectedShow.youtubeKey = best2.key;
            if (callback) callback();
        });
    });
}
...
function updateSpotlight(data, type) {
    ...
    currentSelectedShow.title = title;
    // Note: ang currentSelectedShow.youtubeKey ay hiwalay nang kinukuha ng fetchTrailer()
    // sa fetchFullDetails() dahil mas malawak ang language fallback doon.
}
...
function openPlayer() {
    ...
    if (akingMgaStreamingLinks[showId]) {
        ...
        const searchQuery = encodeURIComponent(`${currentSelectedShow.title || ""} official trailer`);
        container.innerHTML = `
            <div style="text-align:center; padding: 50px;">
                <p style="font-weight:bold; color:red; margin-bottom: 16px;">${t("no_stream")}</p>
                <a href="https://www.youtube.com/results?search_query=${searchQuery}" target="_blank" rel="noopener noreferrer"
                   style="color:#e50914; text-decoration: underline; font-weight:600;">
                   🔎 Search "${currentSelectedShow.title || ""}" on YouTube
                </a>
            </div>`;
    }
